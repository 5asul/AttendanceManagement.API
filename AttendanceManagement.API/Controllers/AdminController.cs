...
public class AdminController(IUnitOfWork unitOfWork) : ControllerBase
{
    ...
    public async Task<IActionResult> AssignUserToWorkTime([FromBody] AssignWorkerToWorkTimeDto dto )
    {
        ...
        try
        {
            await unitOfWork.AdminRepository.AssignEmployeeToWorkTimeAsync(dto.User, dto.WorkTime);
    ...
    public async Task<IActionResult> AddCheckInCheckOutTime([FromBody] AssignCheckInOutDto dto , CheckType type)
    {
    ...
    {
        var records = await unitOfWork.AdminRepository.GetRealTimeAttendanceAsync();
        return Ok(records);
    }
    ...
    {
        var requests = await unitOfWork.AdminRepository.GetAllAbsenceRequestsAsync();
        return Ok(requests);
    }
    ...
    {
        await unitOfWork.AdminRepository.ApproveAbsenceRequestAsync(requestId);
    ...
    {
        await unitOfWork.AdminRepository.RejectAbsenceRequestAsync(requestId);
    ...
    {
        await unitOfWork.AdminRepository.UpdateAttendanceStatusAsync(attendanceStatus, attendanceId);
    ...
    {
        var report = await unitOfWork.AdminRepository.GenerateReportAsync(workerId, yearly, year, month);
        return Ok(report);
    }
    ...
    {
        var (barcodeCode, qrCodeBase64) = await unitOfWork.AdminRepository.GenerateBarcodeCodeAsync();
